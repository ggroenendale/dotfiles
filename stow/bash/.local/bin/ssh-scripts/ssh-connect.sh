...
main() {
    ...
    case "${1:-}" in
        -l|--list)
            "${SCRIPT_DIR}/ssh-list.sh"
            ;;
        -d|--direct)
            if [[ -z "${2:-}" ]]; then
                echo -e "${RED}Error: -d requires an IP or hostname argument${NC}"
                exit 1
            fi
            echo -e "${GREEN}Connecting to ${CYAN}$2${NC}..."
            ssh "$2"
            ;;
        -h|--help)
            show_help
            ;;
        *)
            # Assume it's a hostname to connect directly
            echo -e "${GREEN}Connecting to ${CYAN}$1${NC}..."
            ssh "$1"
            ;;
    esac
}
